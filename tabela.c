...
/* Copia uma string convertendo tudo para minúsculo.
   Usamos isso porque MicroPascal é case-insensitive (X e x são a mesma coisa) */
...
/* Retorna o nome do token como texto para escrever nos arquivos de saída */
...
/* Inicializa a tabela inserindo todas as palavras reservadas do MicroPascal.
   Isso precisa ser feito antes de começar a análise. */
...
/* Busca um lexema na tabela de forma case-insensitive.
   Retorna o índice se achou, -1 se não encontrou. */
...
/* Insere um novo lexema na tabela, mas só se ele ainda não estiver lá.
   A linguagem não permite duplicatas na tabela de símbolos. */
int ts_inserir(TabelaSimbolos *ts, const char *lexema, TipoToken tipo) {
    char lower[MAX_LEXEMA];
    toLowerStr(lexema, lower);

    /* Se já existe, só retorna a posição sem inserir de novo */
