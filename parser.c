...
// felizmente só tem integer e real, então é tranquilo de fazer isso aqui
static No *tipo(void) {
    No *n = no_criar("tipo");
...
void analisarSintatico(FILE *fonte, TabelaSimbolos *ts, const char *arquivoEntrada) {
    ...
    printf("\n[Fase sintatica]\n");
    if (arqProducoes) {
        fprintf(arqProducoes, "Producoes sintaticas\n");
...
// 300 linhas para isso, mas pelo menos e o ultimo trabalho